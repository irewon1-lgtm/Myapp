...
html=html.replace('<script src="./library.js"></script>','<script>window.CHATBOOK_PORTABLE=true;window.CHATBOOK_ART='+json.dumps(art)+';</script><script src="./library.js"></script>')
def inline(m):
 code=(p/m.group(1)).read_text().replace('</script','<\\/script')
