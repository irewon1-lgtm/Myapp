...
if(repo!=='irewon1-lgtm/Myapp'||branch!=='chatbook-app-20260923'||process.env.CHATBOOK_ROUTING_CORRECTION!=='1')throw Error('Initial migration authorization required');
...
const prior=await read('.deployment/live-bootstrap-receipt.json');if(prior?.value?.productionCalls!==1)throw Error('Expected original one-call bootstrap receipt');
const receiptPath='.deployment/live-routing-correction-receipt.json';if(await read(receiptPath))throw Error('Initial deployment already recorded. Refusing another production deployment.');
...
const receipt={kind:'chatbook-live-routing-correction',status:'deployed-awaiting-https-check',run,liveCommit:channel.current.commit,productionCalls:1,totalTransitionCalls:2,createdAt:new Date().toISOString()};
