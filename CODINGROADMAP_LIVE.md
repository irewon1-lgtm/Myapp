...
- 읽기 본문의 기본 글자 크기와 타이포 계층은 페이지마다 동일해야 한다. 페이지별 자동 축소(scale-to-fit)를 금지한다.
- 한 화면(viewport)에는 고정 글자 크기로 읽을 수 있는 만큼만 배치한다. 내용이 많으면 같은 논리 페이지를 2/2, 3/3처럼 추가 화면 페이지로 자동 분할한다.
- 읽기 본문에는 세로 스크롤을 만들지 않는다. 다음 분량은 좌우 페이지 이동으로 이어진다.
- 내용이 적은 화면은 글자를 키우지 않고 기존 제목·본문·예시·정리 블록을 세로로 균등 배치해 과도한 빈 공간을 줄인다.
- 이전/다음 페이지는 좌우 스와이프 및 화면 양끝 탭으로 이동하며, horizontal slide animation을 유지한다.
