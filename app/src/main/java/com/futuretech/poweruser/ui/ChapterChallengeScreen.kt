...
fun ChapterChallengeScreen(
    ...
    val predictionCode = remember(stepIndex, lessonId) {
        challengeVariantCode(lesson.codeSample, ordinal)
    }
    val modifyBaseline = remember(stepIndex, lessonId) {
        challengeVariantCode(lesson.initialPracticeCode, ordinal)
    }
    val writeReference = remember(stepIndex, lessonId) {
        challengeVariantCode(lesson.initialPracticeCode, ordinal)
    }
    ...
    Scaffold(
        ...
        Column(
            ...
            if (submitEvidence.isNotBlank()) {
                ChallengeStatusBox(submitEvidence, passedCurrent)
            }
...
private fun challengeLanguage(raw: String): ProgrammingLanguage =
    runCatching { ProgrammingLanguage.valueOf(raw) }.getOrDefault(ProgrammingLanguage.PYTHON)

private suspend fun gradeModified(
    engine: SandboxedExecutionEngine,
    language: ProgrammingLanguage,
    learner: String,
    baseline: String
): ChallengeGrade {
    if (learner.trim() == baseline.trim()) {
        return ChallengeGrade(false, "! 원본과 같습니다. 값·조건·문장 중 한 곳을 목적 있게 바꾸세요.")
    }
    ...
    return if (failed == 0) {
        ChallengeGrade(true, "✓ 기본 실행 + 숨은 경계 실행 ${hidden.size}/${hidden.size} 통과")
    } else {
        ChallengeGrade(false, "! 기본 실행은 성공했지만 숨은 경계 실행 ${hidden.size - failed}/${hidden.size} 통과")
    }
}

private suspend fun gradeAgainstReference(
    engine: SandboxedExecutionEngine,
    language: ProgrammingLanguage,
    learner: String,
    reference: String
    ...
    for (pair in pairs) {
        ...
        if (learnerEdge.isSuccess && referenceEdge.isSuccess &&
            PracticeTestEngine.normalizeOutput(learnerEdge.output) == PracticeTestEngine.normalizeOutput(referenceEdge.output)
        ) passed++
    }
    return if (passed == pairs.size) {
        ChallengeGrade(true, "✓ 공개 1/1 + 숨은 경계 ${passed}/${pairs.size} 통과")
    } else {
        ChallengeGrade(false, "! 공개 예제는 통과했지만 숨은 경계 ${passed}/${pairs.size} 통과")
    }
}
