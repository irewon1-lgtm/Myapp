...
/**
 * Items 16-20 are intentionally deterministic and local-only.
 * No network or paid AI call is required to build challenge flows, review variants,
 * error-driven weighting, or project stage locks.
 */
object ErrorDrivenPracticeEngine {
    ...
    fun problemTypeFor(note: PersonalErrorNoteEntity): LessonProblemType {
        ...
        return when {
            listOf("fill", "blank", "빈칸", "json comma", "comma", "쉼표").any { it in text } ->
                LessonProblemType.FILL_BLANK
            listOf("debug", "syntax", "runtime", "exception", "indent", "오류", "에러", ">=").any { it in text } ->
                LessonProblemType.DEBUG
            listOf("write", "memory", "직접 작성", "빈 배열", "empty array").any { it in text } ->
                LessonProblemType.WRITE_FROM_MEMORY
            listOf("modify", "run", "수정", "get/post", "get ", "post ").any { it in text } ->
                LessonProblemType.MODIFY_AND_RUN
            listOf("ai_verify", "ai 답", "검증 오답", "hallucination").any { it in text } ->
                LessonProblemType.VERIFY_AI_ANSWER
            else -> LessonProblemType.PREDICT_OUTPUT
        }
    }

    fun weightFor(note: PersonalErrorNoteEntity): Int =
        1 + note.occurrenceCount.coerceIn(1, 8)

    /**
     * Starts with one of every core type, then allocates four extra slots to the
     * user's most frequent real error types. The weighted pool is deterministic so
     * the same local error history always produces the same challenge.
     */
    fun challengeFlow(errorNotes: List<PersonalErrorNoteEntity>): List<LessonProblemType> {
        val weightedPool = errorNotes
            .sortedWith(
                compareByDescending<PersonalErrorNoteEntity> { weightFor(it) }
        ...
        // One final deterministic repair in case a weighted replacement created adjacency.
...
data class ReviewVariation(
    val prompt: String,
    val changedExample: String?,
    val focus: String
)
...
object ReviewVariationEngine {
    fun forItem(item: SpacedRepetitionItemEntity): ReviewVariation {
        ...
        return when (stage) {
            0 -> ReviewVariation(
                prompt = "정의를 보지 않고 이 개념을 본인 말로 설명하세요. 무엇인지와 언제 쓰는지를 함께 말하세요.",
                changedExample = null,
                focus = "단순 문장 암기가 아니라 개념의 역할을 회상합니다."
            )
            1 -> ReviewVariation(
                prompt = "비슷한 개념과 무엇이 다른지 먼저 말한 뒤, 실제 상황 하나를 새로 만들어 설명하세요.",
                changedExample = item.comparison.takeIf { it.isNotBlank() },
                focus = "같은 답을 외우지 않고 구분 기준을 재구성합니다."
            )
            2 -> ReviewVariation(
                prompt = "아래 예시는 원래 학습 때와 입력을 바꾼 변형입니다. 실행 결과나 의미가 어떻게 달라지는지 먼저 예상하세요.",
                changedExample = varyExample(item.example, item.reviewCount),
                focus = "입력·문맥이 바뀌어도 같은 개념을 적용할 수 있는지 확인합니다."
            )
            else -> ReviewVariation(
                prompt = "이 개념을 처음 보는 다른 문제에 적용한다고 가정하세요. 어떤 순서로 판단할지 2~3단계로 말하세요.",
                changedExample = item.analogy.takeIf { it.isNotBlank() },
                focus = "회상이 아니라 전이(transfer)를 확인합니다."
            )
        }
    }
    ...
    private fun varyExample(example: String, seed: Int): String? {
        ...
        if (number != null) {
            ...
            if (replacement != null) {
                return example.replaceRange(number.range, replacement)
            }
        }
        ...
        if (quoted != null) {
            ...
            val replacement = "$quote${body}_변형$quote"
            return example.replaceRange(quoted.range, replacement)
        }
...
object ProjectStageEngine {
    ...
    private fun stage(
        ...
        unlocked = required.all { it in completed }
    )
}
