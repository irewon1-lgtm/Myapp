...
**실행 점검 65-1 — CHAPTER 01 · comparison dispatch는 여섯 비교 연산의 후보 선택 규칙이다**
CHAPTER 01 · comparison dispatch는 여섯 비교 연산의 후보 선택 규칙이다을 이해할 때는 경계값 한 개를 골라 성공 경로와 오류 경로의 차이를 줄 단위로 추적한다. 이어서 값 하나나 호출 순서 하나만 바꿔 결과 차이를 확인하면 CHAPTER 01 · comparison dispatch는 여섯 비교 연산의 후보 선택 규칙이다이 실제 프로그램 상태에 어떤 제약을 주는지 보인다. PART 65 CHAPTER 1의 한 줄 해석은 “규칙을 값과 상태 변화로 확인한다”이다. 같은 증상이 다시 나오도록 재현 절차를 남긴 뒤 원인을 수정한다. 마지막에는 타입·정체성·수명·예외 경계 중 실제 원인이 어디였는지 자기 문장으로 설명해, 단순한 문법 복사가 아니라 동작 원리까지 이해했는지 확인한다.
...
**실행 점검 65-3 — CHAPTER 03 · ordering은 반드시 total order일 필요가 없다**
CHAPTER 03 · ordering은 반드시 total order일 필요가 없다을 이해할 때는 입력 하나를 고정하고 한 번에 한 조건만 바꾸며 결과 차이를 비교한다. 이어서 값 하나나 호출 순서 하나만 바꿔 결과 차이를 확인하면 CHAPTER 03 · ordering은 반드시 total order일 필요가 없다이 실제 프로그램 상태에 어떤 제약을 주는지 보인다. PART 65 CHAPTER 3의 한 줄 해석은 “규칙을 값과 상태 변화로 확인한다”이다. 결과만 맞추지 말고 왜 그 결과가 나왔는지 한 문장으로 설명한다. 마지막에는 타입·정체성·수명·예외 경계 중 실제 원인이 어디였는지 자기 문장으로 설명해, 단순한 문법 복사가 아니라 동작 원리까지 이해했는지 확인한다.
...
**실행 점검 65-4 — CHAPTER 04 · comparison에서 `NotImplemented`는 False와 다르다**
CHAPTER 04 · comparison에서 `NotImplemented`는 False와 다르다을 이해할 때는 관찰할 변수를 두세 개로 줄이고 실행 전 예상값과 실행 후 실제값을 나란히 기록한다. 이어서 값 하나나 호출 순서 하나만 바꿔 결과 차이를 확인하면 CHAPTER 04 · comparison에서 `NotImplemented`는 False와 다르다이 실제 프로그램 상태에 어떤 제약을 주는지 보인다. PART 65 CHAPTER 4의 한 줄 해석은 “규칙을 값과 상태 변화로 확인한다”이다. 수정 후에는 원래 사례와 실패 사례를 모두 재검증한다. 마지막에는 타입·정체성·수명·예외 경계 중 실제 원인이 어디였는지 자기 문장으로 설명해, 단순한 문법 복사가 아니라 동작 원리까지 이해했는지 확인한다.
...
**실행 점검 65-5 — CHAPTER 05 · hash와 equality는 같은 의미 필드를 공유해야 한다**
CHAPTER 05 · hash와 equality는 같은 의미 필드를 공유해야 한다을 이해할 때는 기대 결과를 숫자나 상태로 적고 중간 결과를 출력하거나 로그로 남겨 추측을 줄인다. 이어서 값 하나나 호출 순서 하나만 바꿔 결과 차이를 확인하면 CHAPTER 05 · hash와 equality는 같은 의미 필드를 공유해야 한다이 실제 프로그램 상태에 어떤 제약을 주는지 보인다. PART 65 CHAPTER 5의 한 줄 해석은 “규칙을 값과 상태 변화로 확인한다”이다. 설명이 맞더라도 반대 조건을 넣어 경계가 어디인지 다시 확인한다. 마지막에는 타입·정체성·수명·예외 경계 중 실제 원인이 어디였는지 자기 문장으로 설명해, 단순한 문법 복사가 아니라 동작 원리까지 이해했는지 확인한다.
