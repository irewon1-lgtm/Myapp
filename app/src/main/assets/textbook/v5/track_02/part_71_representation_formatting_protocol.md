...
**검증 시나리오 P71-C1 — CHAPTER 01 · `repr`은 개발자가 객체 상태를 식별할 수 있는 표현을 제공한다**
`CHAPTER 01 · `repr`은 개발자가 객체 상태를 식별할 수 있는 표현을 제공한다` 검증은 기준 입력을 한 개 고정하는 데서 시작한다. P71-C1에서는 `CHAPTER 01 · `repr`은 개발자가 객체 상태를 식별할 수 있는 표현을 제공한다` 실행 직전 상태를 먼저 적고 실행 뒤 값과 비교해 실제 규칙을 확인한다. 두 번째 단계에서는 `CHAPTER 01 · `repr`은 개발자가 객체 상태를 식별할 수 있는 표현을 제공한다`에 대해 반대 조건 입력을 한 개 추가고 다른 코드는 그대로 두어 원인 후보를 하나로 제한한다. 이때 `CHAPTER 01 · `repr`은 개발자가 객체 상태를 식별할 수 있는 표현을 제공한다`의 실행 전후 값을 표로 대조하여 우연한 통과를 배제한다. 예상과 다르면 `CHAPTER 01 · `repr`은 개발자가 객체 상태를 식별할 수 있는 표현을 제공한다`의 타입, 값, 호출 순서, 예외 경계 가운데 최초로 달라진 항목부터 확인하고 최소 수정 뒤 다시 실행한다. P71-C1의 마무리는 예상과 다르면 최초 차이 지점을 추적하는 것이다. 통과 기준은 `CHAPTER 01 · `repr`은 개발자가 객체 상태를 식별할 수 있는 표현을 제공한다`의 정상 사례와 변형 사례가 모두 설명 가능한 결과를 내고 같은 절차를 반복해도 동일한 상태 계약을 유지하는 것이다.
...
**검증 시나리오 P71-C2 — CHAPTER 02 · `str`은 사용자가 읽을 목적의 표현으로 좁힐 수 있다**
`CHAPTER 02 · `str`은 사용자가 읽을 목적의 표현으로 좁힐 수 있다` 검증은 가장 작은 객체 상태로 시작하는 데서 시작한다. P71-C2에서는 `CHAPTER 02 · `str`은 사용자가 읽을 목적의 표현으로 좁힐 수 있다` 실행 직전 상태를 먼저 적고 실행 뒤 값과 비교해 실제 규칙을 확인한다. 두 번째 단계에서는 `CHAPTER 02 · `str`은 사용자가 읽을 목적의 표현으로 좁힐 수 있다`에 대해 속성 하나만 바꿔 재실행고 다른 코드는 그대로 두어 원인 후보를 하나로 제한한다. 이때 `CHAPTER 02 · `str`은 사용자가 읽을 목적의 표현으로 좁힐 수 있다`의 반환값과 부수효과를 따로 기록하여 우연한 통과를 배제한다. 예상과 다르면 `CHAPTER 02 · `str`은 사용자가 읽을 목적의 표현으로 좁힐 수 있다`의 타입, 값, 호출 순서, 예외 경계 가운데 최초로 달라진 항목부터 확인하고 최소 수정 뒤 다시 실행한다. P71-C2의 마무리는 수정 뒤 원래 조건을 다시 회귀 확인하는 것이다. 통과 기준은 `CHAPTER 02 · `str`은 사용자가 읽을 목적의 표현으로 좁힐 수 있다`의 정상 사례와 변형 사례가 모두 설명 가능한 결과를 내고 같은 절차를 반복해도 동일한 상태 계약을 유지하는 것이다.
...
**검증 시나리오 P71-C3 — CHAPTER 03 · `__format__`은 format spec을 타입의 표현 정책으로 해석한다**
`CHAPTER 03 · `__format__`은 format spec을 타입의 표현 정책으로 해석한다` 검증은 경계값을 먼저 지정하는 데서 시작한다. P71-C3에서는 `CHAPTER 03 · `__format__`은 format spec을 타입의 표현 정책으로 해석한다` 실행 직전 상태를 먼저 적고 실행 뒤 값과 비교해 실제 규칙을 확인한다. 두 번째 단계에서는 `CHAPTER 03 · `__format__`은 format spec을 타입의 표현 정책으로 해석한다`에 대해 정상값과 경계값을 연속 실행고 다른 코드는 그대로 두어 원인 후보를 하나로 제한한다. 이때 `CHAPTER 03 · `__format__`은 format spec을 타입의 표현 정책으로 해석한다`의 타입·정체성·수명 변화를 분리 기록하여 우연한 통과를 배제한다. 예상과 다르면 `CHAPTER 03 · `__format__`은 format spec을 타입의 표현 정책으로 해석한다`의 타입, 값, 호출 순서, 예외 경계 가운데 최초로 달라진 항목부터 확인하고 최소 수정 뒤 다시 실행한다. P71-C3의 마무리는 결과를 설명할 수 있을 때 종료하는 것이다. 통과 기준은 `CHAPTER 03 · `__format__`은 format spec을 타입의 표현 정책으로 해석한다`의 정상 사례와 변형 사례가 모두 설명 가능한 결과를 내고 같은 절차를 반복해도 동일한 상태 계약을 유지하는 것이다.
...
**검증 시나리오 P71-C4 — CHAPTER 04 · f-string conversion은 `!r`·`!s`·format spec의 적용 순서를 구분한다**
`CHAPTER 04 · f-string conversion은 `!r`·`!s`·format spec의 적용 순서를 구분한다` 검증은 정상 경로를 먼저 재현하는 데서 시작한다. P71-C4에서는 `CHAPTER 04 · f-string conversion은 `!r`·`!s`·format spec의 적용 순서를 구분한다` 실행 직전 상태를 먼저 적고 실행 뒤 값과 비교해 실제 규칙을 확인한다. 두 번째 단계에서는 `CHAPTER 04 · f-string conversion은 `!r`·`!s`·format spec의 적용 순서를 구분한다`에 대해 오류 경로 하나를 의도적으로 만든다고 다른 코드는 그대로 두어 원인 후보를 하나로 제한한다. 이때 `CHAPTER 04 · f-string conversion은 `!r`·`!s`·format spec의 적용 순서를 구분한다`의 예외 종류와 직전 상태를 함께 남긴다하여 우연한 통과를 배제한다. 예상과 다르면 `CHAPTER 04 · f-string conversion은 `!r`·`!s`·format spec의 적용 순서를 구분한다`의 타입, 값, 호출 순서, 예외 경계 가운데 최초로 달라진 항목부터 확인하고 최소 수정 뒤 다시 실행한다. P71-C4의 마무리는 복구 후 같은 오류가 다시 재현되지 않는지 검사하는 것이다. 통과 기준은 `CHAPTER 04 · f-string conversion은 `!r`·`!s`·format spec의 적용 순서를 구분한다`의 정상 사례와 변형 사례가 모두 설명 가능한 결과를 내고 같은 절차를 반복해도 동일한 상태 계약을 유지하는 것이다.
...
**검증 시나리오 P71-C7 — CHAPTER 07 · recursive object graph의 `repr`은 무한 재귀와 폭발적 출력을 피해야 한다**
`CHAPTER 07 · recursive object graph의 `repr`은 무한 재귀와 폭발적 출력을 피해야 한다` 검증은 성공 사례를 기준선으로 저장하는 데서 시작한다. P71-C7에서는 `CHAPTER 07 · recursive object graph의 `repr`은 무한 재귀와 폭발적 출력을 피해야 한다` 실행 직전 상태를 먼저 적고 실행 뒤 값과 비교해 실제 규칙을 확인한다. 두 번째 단계에서는 `CHAPTER 07 · recursive object graph의 `repr`은 무한 재귀와 폭발적 출력을 피해야 한다`에 대해 실패 조건은 하나만 주입고 다른 코드는 그대로 두어 원인 후보를 하나로 제한한다. 이때 `CHAPTER 07 · recursive object graph의 `repr`은 무한 재귀와 폭발적 출력을 피해야 한다`의 로그 시각과 상태 식별자를 맞춰 본다하여 우연한 통과를 배제한다. 예상과 다르면 `CHAPTER 07 · recursive object graph의 `repr`은 무한 재귀와 폭발적 출력을 피해야 한다`의 타입, 값, 호출 순서, 예외 경계 가운데 최초로 달라진 항목부터 확인하고 최소 수정 뒤 다시 실행한다. P71-C7의 마무리는 성공·실패 모두 결정적으로 끝나는지 확인하는 것이다. 통과 기준은 `CHAPTER 07 · recursive object graph의 `repr`은 무한 재귀와 폭발적 출력을 피해야 한다`의 정상 사례와 변형 사례가 모두 설명 가능한 결과를 내고 같은 절차를 반복해도 동일한 상태 계약을 유지하는 것이다.
