...
**실행 점검 71-1 — CHAPTER 01 · `repr`은 개발자가 객체 상태를 식별할 수 있는 표현을 제공한다**
CHAPTER 01 · `repr`은 개발자가 객체 상태를 식별할 수 있는 표현을 제공한다을 이해할 때는 입력 하나를 고정하고 타입과 값, 상태 전이를 분리해서 본다. 그다음 값 하나, 호출 순서 하나, 경계 조건 하나만 바꿔 CHAPTER 01 · `repr`은 개발자가 객체 상태를 식별할 수 있는 표현을 제공한다의 규칙이 실제 상태에 어떤 변화를 만드는지 확인한다. PART 71 CHAPTER 1에서는 결과를 맞히는 것보다 예상과 실제의 차이를 설명하는 과정이 중요하다. 코드 모양보다 값의 변화와 객체의 수명을 중심으로 원인을 좁힌다. P71-C01 검증의 마지막 단계에서는 결과값보다 원인을 설명한다. P71-C01의 타입·정체성·수명·예외 경계를 다시 대조하고, 반대 조건에서도 같은 규칙이 유지되는지 확인한다.
...
**실행 점검 71-2 — CHAPTER 02 · `str`은 사용자가 읽을 목적의 표현으로 좁힐 수 있다**
CHAPTER 02 · `str`은 사용자가 읽을 목적의 표현으로 좁힐 수 있다을 이해할 때는 반례 하나를 먼저 적고 중간 결과를 출력하거나 로그로 남겨 추측을 줄인다. 그다음 값 하나, 호출 순서 하나, 경계 조건 하나만 바꿔 CHAPTER 02 · `str`은 사용자가 읽을 목적의 표현으로 좁힐 수 있다의 규칙이 실제 상태에 어떤 변화를 만드는지 확인한다. PART 71 CHAPTER 2에서는 결과를 맞히는 것보다 예상과 실제의 차이를 설명하는 과정이 중요하다. 설명이 맞더라도 반대 조건을 넣어 경계가 어디인지 다시 확인한다. P71-C02에서는 실행 결과만 맞는 것으로 끝내지 않는다. P71-C02의 타입과 정체성, 자원 수명, 오류 경계를 각각 짚고 다른 코드에서도 같은 판단 기준을 재현한다.
...
**실행 점검 71-3 — CHAPTER 03 · `__format__`은 format spec을 타입의 표현 정책으로 해석한다**
CHAPTER 03 · `__format__`은 format spec을 타입의 표현 정책으로 해석한다을 이해할 때는 경계값 한 개를 골라 변경 전후의 반환값과 부수효과를 따로 확인한다. 그다음 값 하나, 호출 순서 하나, 경계 조건 하나만 바꿔 CHAPTER 03 · `__format__`은 format spec을 타입의 표현 정책으로 해석한다의 규칙이 실제 상태에 어떤 변화를 만드는지 확인한다. PART 71 CHAPTER 3에서는 결과를 맞히는 것보다 예상과 실제의 차이를 설명하는 과정이 중요하다. 우연히 통과한 한 번의 실행보다 반복 가능한 관찰을 통과 기준으로 삼는다. P71-C03 학습을 끝내기 전에 결정 요인을 분리한다. P71-C03의 타입·객체 정체성·수명·예외 처리 중 실제 원인을 지목하고, 새 예제에서도 같은 근거로 결과를 예측한다.
...
**실행 점검 71-4 — CHAPTER 04 · f-string conversion은 `!r`·`!s`·format spec의 적용 순서를 구분한다**
CHAPTER 04 · f-string conversion은 `!r`·`!s`·format spec의 적용 순서를 구분한다을 이해할 때는 관찰할 변수를 두세 개로 줄이고 한 번에 한 조건만 바꾸며 결과 차이를 비교한다. 그다음 값 하나, 호출 순서 하나, 경계 조건 하나만 바꿔 CHAPTER 04 · f-string conversion은 `!r`·`!s`·format spec의 적용 순서를 구분한다의 규칙이 실제 상태에 어떤 변화를 만드는지 확인한다. PART 71 CHAPTER 4에서는 결과를 맞히는 것보다 예상과 실제의 차이를 설명하는 과정이 중요하다. 결과만 맞추지 말고 왜 그 결과가 나왔는지 한 문장으로 설명한다. P71-C04 마무리에서는 동작을 만든 핵심 조건을 기록한다. P71-C04의 값 타입과 객체 관계, 생명주기, 예외 위치를 다시 비교한 뒤 변형 예제에서도 판단이 흔들리지 않는지 확인한다.
...
**실행 점검 71-7 — CHAPTER 07 · recursive object graph의 `repr`은 무한 재귀와 폭발적 출력을 피해야 한다**
CHAPTER 07 · recursive object graph의 `repr`은 무한 재귀와 폭발적 출력을 피해야 한다을 이해할 때는 기대 결과를 숫자나 상태로 적고 실행 전 예상값과 실행 후 실제값을 나란히 기록한다. 그다음 값 하나, 호출 순서 하나, 경계 조건 하나만 바꿔 CHAPTER 07 · recursive object graph의 `repr`은 무한 재귀와 폭발적 출력을 피해야 한다의 규칙이 실제 상태에 어떤 변화를 만드는지 확인한다. PART 71 CHAPTER 7에서는 결과를 맞히는 것보다 예상과 실제의 차이를 설명하는 과정이 중요하다. 수정 후에는 원래 사례와 실패 사례를 모두 재검증한다. P71-C07에서는 실행 결과만 맞는 것으로 끝내지 않는다. P71-C07의 타입과 정체성, 자원 수명, 오류 경계를 각각 짚고 다른 코드에서도 같은 판단 기준을 재현한다.
