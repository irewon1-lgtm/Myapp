...
**운영 검증 97-1 — CHAPTER 01 · timeout은 한 operation의 최대 대기이고 deadline은 전체 작업의 종료 시점이다**
CHAPTER 01 · timeout은 한 operation의 최대 대기이고 deadline은 전체 작업의 종료 시점이다을 검증할 때는 제한시간과 재시도 횟수를 명시하고 연속 실패 뒤 정상 요청을 보낸다. PART 97 CHAPTER 1의 핵심 관찰값으로 중복 반영과 누락을 별도 지표로 센다. P97-C01 복구 단계에서는 동일한 식별자로 요청을 재전송한다. P97-C01의 이미 완료된 효과는 유지되고 미완료 부분만 이어지며, 재실행 결과가 최초 정상 경로와 같은 최종 상태로 수렴해야 한다. P97-C01: 회귀 테스트는 성공률 하나로 끝내지 않는다. 동일 입력의 재실행 결과, 재시도 횟수, 중복 반영 여부, 종료 상태를 함께 확인해 모든 값이 계약 범위 안에 있어야 한다.
...
**운영 검증 97-2 — CHAPTER 02 · elapsed-time 계산에는 wall clock보다 monotonic clock이 적합하다**
CHAPTER 02 · elapsed-time 계산에는 wall clock보다 monotonic clock이 적합하다을 검증할 때는 중복 여부를 판정할 키를 먼저 확인하고 복구 직후 같은 요청을 재전송한다. PART 97 CHAPTER 2의 핵심 관찰값으로 최종 상태와 처리 횟수를 함께 확인한다. P97-C02의 실패 주입 결과는 다음 정상 실행과 연결해서 본다. 같은 입력을 다시 처리할 때 P97-C02의 상태 머신이 이전 오류를 끌고 가지 않고 정해진 완료 상태로 돌아오는지 확인한다. P97-C02: 수정 전후는 처리 건수와 재시도 횟수를 분리해 비교한다. 같은 입력을 세 번 반복했을 때 최종 반영 수가 변하지 않고 실패 이유가 동일한 식별자로 남으면 검증을 통과한다.
...
**운영 검증 97-5 — CHAPTER 05 · async timeout은 underlying work의 cancellation semantics와 연결된다**
CHAPTER 05 · async timeout은 underlying work의 cancellation semantics와 연결된다을 검증할 때는 성공 기준을 수치나 상태로 정하고 처리 중간에 프로세스를 중단한다. PART 97 CHAPTER 5의 핵심 관찰값으로 대기열 길이와 거부된 요청 수를 같이 본다. P97-C05에서는 장애를 제거한 뒤 동일 입력으로 기준 결과를 다시 만든다. P97-C05 재실행에서 완료 항목이 중복되지 않고 실패 항목만 정책대로 이어지며 최종 상태가 정상 실행과 같아야 한다. P97-C05: 변경 효과는 성공 플래그보다 중복 반영 수와 미처리 항목 수로 판단한다. 재실행 뒤 누락이 줄고 이미 완료된 항목이 다시 처리되지 않으며 종료 상태가 명확해야 한다.
