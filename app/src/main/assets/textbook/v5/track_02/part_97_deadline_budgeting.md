...
CHAPTER 02 · elapsed-time 계산에는 wall clock보다 monotonic clock이 적합하다을 검증할 때는 중복 여부를 판정할 키를 먼저 확인하고 복구 직후 같은 요청을 재전송한다. PART 97 CHAPTER 2의 핵심 관찰값으로 최종 상태와 처리 횟수를 함께 확인한다. 실패를 주입한 뒤 정상 경로로 돌아왔을 때 같은 입력을 다시 처리해 멱등성, 복구 가능성, 상태 일관성을 확인한다. 수정 전후를 비교할 때는 성공 여부만 보지 말고 재시도 횟수, 중복 처리, 누락, 남은 작업을 함께 본다. 통과 기준은 반복 실행에서도 결과가 안정적이고, 실패가 생겨도 손실이나 무한 반복 없이 추적 가능한 상태로 종료되는 것이다.
...
CHAPTER 05 · async timeout은 underlying work의 cancellation semantics와 연결된다을 검증할 때는 성공 기준을 수치나 상태로 정하고 처리 중간에 프로세스를 중단한다. PART 97 CHAPTER 5의 핵심 관찰값으로 대기열 길이와 거부된 요청 수를 같이 본다. 실패를 주입한 뒤 정상 경로로 돌아왔을 때 같은 입력을 다시 처리해 멱등성, 복구 가능성, 상태 일관성을 확인한다. 수정 전후를 비교할 때는 성공 여부만 보지 말고 재시도 횟수, 중복 처리, 누락, 남은 작업을 함께 본다. 통과 기준은 반복 실행에서도 결과가 안정적이고, 실패가 생겨도 손실이나 무한 반복 없이 추적 가능한 상태로 종료되는 것이다.
