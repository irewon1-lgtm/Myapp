...
PART 67의 **PART 67 · Attribute lookup routing — `__getattribute__`·`__getattr__`·descriptor 우선순위를 추적하기**에서 결과가 예상과 다르면 문법을 먼저 바꾸지 않는다. PART 67 · Attribute lookup routing — `__getattribute__`·`__getattr__`·descriptor 우선순위를 추적하기의 protocol 호출 순서, 현재 상태, 지켜야 할 계약을 차례로 기록한 뒤 최초로 예상과 달라진 지점을 찾는다. 이 절차를 거치면 PART 67의 동작이 우연히 맞은 것인지, 규칙을 설명할 수 있어 재현 가능한 것인지 구분할 수 있다.
...
PART 67의 **PART 67 · Attribute lookup routing — `__getattribute__`·`__getattr__`·descriptor 우선순위를 추적하기**를 마무리할 때는 입력값, 실제로 선택된 규칙, 그 규칙이 만든 상태 변화, 마지막 결과나 예외를 연결해서 요약한다. 특히 PART 67 · Attribute lookup routing — `__getattribute__`·`__getattr__`·descriptor 우선순위를 추적하기에서 관찰한 상태 전이를 자기 말로 설명하고 같은 입력을 다시 넣었을 때 같은 결과가 나오는지 확인하면, PART 67의 실행 모델을 암기가 아니라 검증 가능한 형태로 이해한 것이다.
