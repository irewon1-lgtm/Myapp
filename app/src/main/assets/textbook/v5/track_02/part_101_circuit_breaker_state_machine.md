...
**검증 시나리오 P101-C2 — CHAPTER 02 · CLOSED·OPEN·HALF_OPEN은 호출 허용 범위를 바꾸는 명시적 상태다**
`CHAPTER 02 · CLOSED·OPEN·HALF_OPEN은 호출 허용 범위를 바꾸는 명시적 상태다` 검증은 반례부터 하나 만든다하는 데서 시작한다. P101-C2에서는 `CHAPTER 02 · CLOSED·OPEN·HALF_OPEN은 호출 허용 범위를 바꾸는 명시적 상태다` 실행 직전 상태를 먼저 적고 실행 뒤 값과 비교해 실제 규칙을 확인한다. 두 번째 단계에서는 `CHAPTER 02 · CLOSED·OPEN·HALF_OPEN은 호출 허용 범위를 바꾸는 명시적 상태다`에 대해 반례와 정상례의 차이만 좁힌다고 다른 코드는 그대로 두어 원인 후보를 하나로 제한한다. 이때 `CHAPTER 02 · CLOSED·OPEN·HALF_OPEN은 호출 허용 범위를 바꾸는 명시적 상태다`의 중간 값을 직접 출력해 추측을 줄인다하여 우연한 통과를 배제한다. 예상과 다르면 `CHAPTER 02 · CLOSED·OPEN·HALF_OPEN은 호출 허용 범위를 바꾸는 명시적 상태다`의 타입, 값, 호출 순서, 예외 경계 가운데 최초로 달라진 항목부터 확인하고 최소 수정 뒤 다시 실행한다. P101-C2의 마무리는 수정 후 두 사례를 같은 순서로 재검증하는 것이다. 통과 기준은 `CHAPTER 02 · CLOSED·OPEN·HALF_OPEN은 호출 허용 범위를 바꾸는 명시적 상태다`의 정상 사례와 변형 사례가 모두 설명 가능한 결과를 내고 같은 절차를 반복해도 동일한 상태 계약을 유지하는 것이다.
...
**검증 시나리오 P101-C6 — CHAPTER 06 · HALF_OPEN probe는 recovery를 확인하되 다시 overload를 만들지 않아야 한다**
`CHAPTER 06 · HALF_OPEN probe는 recovery를 확인하되 다시 overload를 만들지 않아야 한다` 검증은 정상 경로를 먼저 재현하는 데서 시작한다. P101-C6에서는 `CHAPTER 06 · HALF_OPEN probe는 recovery를 확인하되 다시 overload를 만들지 않아야 한다` 실행 직전 상태를 먼저 적고 실행 뒤 값과 비교해 실제 규칙을 확인한다. 두 번째 단계에서는 `CHAPTER 06 · HALF_OPEN probe는 recovery를 확인하되 다시 overload를 만들지 않아야 한다`에 대해 오류 경로 하나를 의도적으로 만든다고 다른 코드는 그대로 두어 원인 후보를 하나로 제한한다. 이때 `CHAPTER 06 · HALF_OPEN probe는 recovery를 확인하되 다시 overload를 만들지 않아야 한다`의 예외 종류와 직전 상태를 함께 남긴다하여 우연한 통과를 배제한다. 예상과 다르면 `CHAPTER 06 · HALF_OPEN probe는 recovery를 확인하되 다시 overload를 만들지 않아야 한다`의 타입, 값, 호출 순서, 예외 경계 가운데 최초로 달라진 항목부터 확인하고 최소 수정 뒤 다시 실행한다. P101-C6의 마무리는 복구 후 같은 오류가 다시 재현되지 않는지 검사하는 것이다. 통과 기준은 `CHAPTER 06 · HALF_OPEN probe는 recovery를 확인하되 다시 overload를 만들지 않아야 한다`의 정상 사례와 변형 사례가 모두 설명 가능한 결과를 내고 같은 절차를 반복해도 동일한 상태 계약을 유지하는 것이다.
