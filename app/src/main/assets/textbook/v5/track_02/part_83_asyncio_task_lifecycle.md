...
**검증 시나리오 P83-C1 — CHAPTER 01 · coroutine object와 Task는 실행 상태의 소유권이 다르다**
`CHAPTER 01 · coroutine object와 Task는 실행 상태의 소유권이 다르다` 검증은 호출 순서를 단순화하는 데서 시작한다. P83-C1에서는 `CHAPTER 01 · coroutine object와 Task는 실행 상태의 소유권이 다르다` 실행 직전 상태를 먼저 적고 실행 뒤 값과 비교해 실제 규칙을 확인한다. 두 번째 단계에서는 `CHAPTER 01 · coroutine object와 Task는 실행 상태의 소유권이 다르다`에 대해 순서 하나만 뒤집어 차이를 본다고 다른 코드는 그대로 두어 원인 후보를 하나로 제한한다. 이때 `CHAPTER 01 · coroutine object와 Task는 실행 상태의 소유권이 다르다`의 호출 전후의 상태 전이를 번호로 남긴다하여 우연한 통과를 배제한다. 예상과 다르면 `CHAPTER 01 · coroutine object와 Task는 실행 상태의 소유권이 다르다`의 타입, 값, 호출 순서, 예외 경계 가운데 최초로 달라진 항목부터 확인하고 최소 수정 뒤 다시 실행한다. P83-C1의 마무리는 다른 순서에서도 계약이 유지되는지 확인하는 것이다. 통과 기준은 `CHAPTER 01 · coroutine object와 Task는 실행 상태의 소유권이 다르다`의 정상 사례와 변형 사례가 모두 설명 가능한 결과를 내고 같은 절차를 반복해도 동일한 상태 계약을 유지하는 것이다.
...
**검증 시나리오 P83-C2 — CHAPTER 02 · task scheduling은 즉시 완료가 아니라 event loop에 실행 기회를 등록하는 일이다**
`CHAPTER 02 · task scheduling은 즉시 완료가 아니라 event loop에 실행 기회를 등록하는 일이다` 검증은 입력 크기를 고정하는 데서 시작한다. P83-C2에서는 `CHAPTER 02 · task scheduling은 즉시 완료가 아니라 event loop에 실행 기회를 등록하는 일이다` 실행 직전 상태를 먼저 적고 실행 뒤 값과 비교해 실제 규칙을 확인한다. 두 번째 단계에서는 `CHAPTER 02 · task scheduling은 즉시 완료가 아니라 event loop에 실행 기회를 등록하는 일이다`에 대해 변형은 한 요소만 허용고 다른 코드는 그대로 두어 원인 후보를 하나로 제한한다. 이때 `CHAPTER 02 · task scheduling은 즉시 완료가 아니라 event loop에 실행 기회를 등록하는 일이다`의 측정값을 여러 번 모아 분포를 비교하여 우연한 통과를 배제한다. 예상과 다르면 `CHAPTER 02 · task scheduling은 즉시 완료가 아니라 event loop에 실행 기회를 등록하는 일이다`의 타입, 값, 호출 순서, 예외 경계 가운데 최초로 달라진 항목부터 확인하고 최소 수정 뒤 다시 실행한다. P83-C2의 마무리는 워밍업과 측정 자체의 비용을 분리하는 것이다. 통과 기준은 `CHAPTER 02 · task scheduling은 즉시 완료가 아니라 event loop에 실행 기회를 등록하는 일이다`의 정상 사례와 변형 사례가 모두 설명 가능한 결과를 내고 같은 절차를 반복해도 동일한 상태 계약을 유지하는 것이다.
...
**검증 시나리오 P83-C5 — CHAPTER 05 · cancellation 처리 중에도 cleanup을 수행하고 취소 의미는 보존해야 한다**
`CHAPTER 05 · cancellation 처리 중에도 cleanup을 수행하고 취소 의미는 보존해야 한다` 검증은 기준 입력을 한 개 고정하는 데서 시작한다. P83-C5에서는 `CHAPTER 05 · cancellation 처리 중에도 cleanup을 수행하고 취소 의미는 보존해야 한다` 실행 직전 상태를 먼저 적고 실행 뒤 값과 비교해 실제 규칙을 확인한다. 두 번째 단계에서는 `CHAPTER 05 · cancellation 처리 중에도 cleanup을 수행하고 취소 의미는 보존해야 한다`에 대해 반대 조건 입력을 한 개 추가고 다른 코드는 그대로 두어 원인 후보를 하나로 제한한다. 이때 `CHAPTER 05 · cancellation 처리 중에도 cleanup을 수행하고 취소 의미는 보존해야 한다`의 실행 전후 값을 표로 대조하여 우연한 통과를 배제한다. 예상과 다르면 `CHAPTER 05 · cancellation 처리 중에도 cleanup을 수행하고 취소 의미는 보존해야 한다`의 타입, 값, 호출 순서, 예외 경계 가운데 최초로 달라진 항목부터 확인하고 최소 수정 뒤 다시 실행한다. P83-C5의 마무리는 예상과 다르면 최초 차이 지점을 추적하는 것이다. 통과 기준은 `CHAPTER 05 · cancellation 처리 중에도 cleanup을 수행하고 취소 의미는 보존해야 한다`의 정상 사례와 변형 사례가 모두 설명 가능한 결과를 내고 같은 절차를 반복해도 동일한 상태 계약을 유지하는 것이다.
