...
CHAPTER 03 · immutable built-in subclass는 값 자체를 `__new__` 단계에서 결정해야 한다을 이해할 때는 관찰할 변수를 두세 개로 줄이고 한 번에 한 조건만 바꾸며 결과 차이를 비교한다. 그다음 값 하나, 호출 순서 하나, 경계 조건 하나만 바꿔 CHAPTER 03 · immutable built-in subclass는 값 자체를 `__new__` 단계에서 결정해야 한다의 규칙이 실제 상태에 어떤 변화를 만드는지 확인한다. PART 72 CHAPTER 3에서는 결과를 맞히는 것보다 예상과 실제의 차이를 설명하는 과정이 중요하다. 결과만 맞추지 말고 왜 그 결과가 나왔는지 한 문장으로 설명한다. P72-C03 학습을 끝내기 전에 결정 요인을 분리한다. P72-C03의 타입·객체 정체성·수명·예외 처리 중 실제 원인을 지목하고, 새 예제에서도 같은 근거로 결과를 예측한다.
