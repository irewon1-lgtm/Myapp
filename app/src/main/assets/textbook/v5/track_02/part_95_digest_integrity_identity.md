...
CHAPTER 02 · 큰 파일은 전체를 memory에 올리지 않고 streaming hash로 계산한다을 검증할 때는 부분 성공을 구분할 상태 값을 정하고 중복 요청을 한 번 넣는다. PART 95 CHAPTER 2의 핵심 관찰값으로 재시도 로그와 실제 반영 건수를 비교한다. 실패를 주입한 뒤 정상 경로로 돌아왔을 때 같은 입력을 다시 처리해 멱등성, 복구 가능성, 상태 일관성을 확인한다. 수정 전후를 비교할 때는 성공 여부만 보지 말고 재시도 횟수, 중복 처리, 누락, 남은 작업을 함께 본다. 통과 기준은 반복 실행에서도 결과가 안정적이고, 실패가 생겨도 손실이나 무한 반복 없이 추적 가능한 상태로 종료되는 것이다.
...
CHAPTER 05 · content-addressed identity는 내용이 같으면 같은 key가 된다는 성질을 사용한다을 검증할 때는 재시작 전후를 비교할 값을 고르고 부분 배치를 실패시킨다. PART 95 CHAPTER 5의 핵심 관찰값으로 실패 원인과 복구 결과가 같은 식별자로 이어지는지 확인한다. 실패를 주입한 뒤 정상 경로로 돌아왔을 때 같은 입력을 다시 처리해 멱등성, 복구 가능성, 상태 일관성을 확인한다. 수정 전후를 비교할 때는 성공 여부만 보지 말고 재시도 횟수, 중복 처리, 누락, 남은 작업을 함께 본다. 통과 기준은 반복 실행에서도 결과가 안정적이고, 실패가 생겨도 손실이나 무한 반복 없이 추적 가능한 상태로 종료되는 것이다.
...
CHAPTER 07 · algorithm과 representation version을 identity에 포함하면 migration이 가능해진다을 검증할 때는 성공 기준을 수치나 상태로 정하고 처리 중간에 프로세스를 중단한다. PART 95 CHAPTER 7의 핵심 관찰값으로 대기열 길이와 거부된 요청 수를 같이 본다. 실패를 주입한 뒤 정상 경로로 돌아왔을 때 같은 입력을 다시 처리해 멱등성, 복구 가능성, 상태 일관성을 확인한다. 수정 전후를 비교할 때는 성공 여부만 보지 말고 재시도 횟수, 중복 처리, 누락, 남은 작업을 함께 본다. 통과 기준은 반복 실행에서도 결과가 안정적이고, 실패가 생겨도 손실이나 무한 반복 없이 추적 가능한 상태로 종료되는 것이다.
