...
CHAPTER 03 · exponential backoff와 jitter는 동시 retry 폭주를 줄인다을 검증할 때는 요청과 처리 상태를 연결할 식별자를 정하고 같은 식별자로 다시 전달한다. PART 98 CHAPTER 3의 핵심 관찰값으로 종료 전 남은 작업과 종료 후 상태를 비교한다. 실패를 주입한 뒤 정상 경로로 돌아왔을 때 같은 입력을 다시 처리해 멱등성, 복구 가능성, 상태 일관성을 확인한다. 수정 전후를 비교할 때는 성공 여부만 보지 말고 재시도 횟수, 중복 처리, 누락, 남은 작업을 함께 본다. 통과 기준은 반복 실행에서도 결과가 안정적이고, 실패가 생겨도 손실이나 무한 반복 없이 추적 가능한 상태로 종료되는 것이다.
