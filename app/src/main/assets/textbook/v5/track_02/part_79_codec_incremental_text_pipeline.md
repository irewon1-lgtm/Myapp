...
**검증 시나리오 P79-C1 — CHAPTER 01 · text와 bytes는 의미 층이 다르므로 경계를 명시한다**
`CHAPTER 01 · text와 bytes는 의미 층이 다르므로 경계를 명시한다` 검증은 기준 입력을 한 개 고정하는 데서 시작한다. P79-C1에서는 `CHAPTER 01 · text와 bytes는 의미 층이 다르므로 경계를 명시한다` 실행 직전 상태를 먼저 적고 실행 뒤 값과 비교해 실제 규칙을 확인한다. 두 번째 단계에서는 `CHAPTER 01 · text와 bytes는 의미 층이 다르므로 경계를 명시한다`에 대해 반대 조건 입력을 한 개 추가고 다른 코드는 그대로 두어 원인 후보를 하나로 제한한다. 이때 `CHAPTER 01 · text와 bytes는 의미 층이 다르므로 경계를 명시한다`의 실행 전후 값을 표로 대조하여 우연한 통과를 배제한다. 예상과 다르면 `CHAPTER 01 · text와 bytes는 의미 층이 다르므로 경계를 명시한다`의 타입, 값, 호출 순서, 예외 경계 가운데 최초로 달라진 항목부터 확인하고 최소 수정 뒤 다시 실행한다. P79-C1의 마무리는 예상과 다르면 최초 차이 지점을 추적하는 것이다. 통과 기준은 `CHAPTER 01 · text와 bytes는 의미 층이 다르므로 경계를 명시한다`의 정상 사례와 변형 사례가 모두 설명 가능한 결과를 내고 같은 절차를 반복해도 동일한 상태 계약을 유지하는 것이다.
