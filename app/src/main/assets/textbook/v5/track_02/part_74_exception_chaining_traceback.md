...
**실행 점검 74-1 — CHAPTER 01 · exception context는 처리 중 새 exception이 발생했음을 연결한다**
CHAPTER 01 · exception context는 처리 중 새 exception이 발생했음을 연결한다을 이해할 때는 관찰할 변수를 두세 개로 줄이고 한 번에 한 조건만 바꾸며 결과 차이를 비교한다. 그다음 값 하나, 호출 순서 하나, 경계 조건 하나만 바꿔 CHAPTER 01 · exception context는 처리 중 새 exception이 발생했음을 연결한다의 규칙이 실제 상태에 어떤 변화를 만드는지 확인한다. PART 74 CHAPTER 1에서는 결과를 맞히는 것보다 예상과 실제의 차이를 설명하는 과정이 중요하다. 결과만 맞추지 말고 왜 그 결과가 나왔는지 한 문장으로 설명한다. P74-C01 최종 확인은 규칙을 자기 말로 다시 설명하는 것이다. P74-C01의 값 종류와 객체 동일성, 수명 변화, 예외 발생 지점을 다른 입력에 적용해 같은 결론이 나오는지 본다.
...
**실행 점검 74-2 — CHAPTER 02 · `raise ... from ...`은 명시적 cause를 만들어 번역 관계를 표현한다**
CHAPTER 02 · `raise ... from ...`은 명시적 cause를 만들어 번역 관계를 표현한다을 이해할 때는 호출 순서를 단순화하고 예외가 생긴 지점과 그 직전 상태를 함께 적는다. 그다음 값 하나, 호출 순서 하나, 경계 조건 하나만 바꿔 CHAPTER 02 · `raise ... from ...`은 명시적 cause를 만들어 번역 관계를 표현한다의 규칙이 실제 상태에 어떤 변화를 만드는지 확인한다. PART 74 CHAPTER 2에서는 결과를 맞히는 것보다 예상과 실제의 차이를 설명하는 과정이 중요하다. 예상이 틀리면 규칙을 다시 읽고 최소 수정으로 재실행한다. P74-C02의 통과 기준은 결과를 외우는 것이 아니다. P74-C02의 동작을 타입, 정체성, 수명, 오류 경계로 나눠 설명하고 비슷한 코드에서도 동일한 규칙을 찾아낼 수 있어야 한다.
...
**실행 점검 74-3 — CHAPTER 03 · `from None`은 context 표시를 억제하지만 원인 자체를 없애는 설계 도구는 아니다**
CHAPTER 03 · `from None`은 context 표시를 억제하지만 원인 자체를 없애는 설계 도구는 아니다을 이해할 때는 가장 작은 객체를 만들고 성공 경로와 오류 경로의 차이를 줄 단위로 추적한다. 그다음 값 하나, 호출 순서 하나, 경계 조건 하나만 바꿔 CHAPTER 03 · `from None`은 context 표시를 억제하지만 원인 자체를 없애는 설계 도구는 아니다의 규칙이 실제 상태에 어떤 변화를 만드는지 확인한다. PART 74 CHAPTER 3에서는 결과를 맞히는 것보다 예상과 실제의 차이를 설명하는 과정이 중요하다. 같은 증상이 다시 나오도록 재현 절차를 남긴 뒤 원인을 수정한다. P74-C03은 마지막에 설명 가능성을 검사한다. P74-C03의 결과를 만든 타입·정체성·수명·예외 조건을 명시하고, 다른 코드 조각에서도 같은 조건을 스스로 찾아 적용한다.
...
**실행 점검 74-5 — CHAPTER 05 · bare `raise`는 현재 exception의 traceback을 보존하는 재발생 경로다**
CHAPTER 05 · bare `raise`는 현재 exception의 traceback을 보존하는 재발생 경로다을 이해할 때는 정상 사례 하나를 정한 뒤 같은 입력을 두 번 실행해 결과가 안정적인지 확인한다. 그다음 값 하나, 호출 순서 하나, 경계 조건 하나만 바꿔 CHAPTER 05 · bare `raise`는 현재 exception의 traceback을 보존하는 재발생 경로다의 규칙이 실제 상태에 어떤 변화를 만드는지 확인한다. PART 74 CHAPTER 5에서는 결과를 맞히는 것보다 예상과 실제의 차이를 설명하는 과정이 중요하다. 다른 입력에서도 같은 계약이 유지되는지 확인해야 검증이 끝난다. P74-C05에서는 실행 결과만 맞는 것으로 끝내지 않는다. P74-C05의 타입과 정체성, 자원 수명, 오류 경계를 각각 짚고 다른 코드에서도 같은 판단 기준을 재현한다.
...
**실행 점검 74-7 — CHAPTER 07 · boundary translation은 내부 exception vocabulary를 public domain vocabulary로 바꾼다**
CHAPTER 07 · boundary translation은 내부 exception vocabulary를 public domain vocabulary로 바꾼다을 이해할 때는 반례 하나를 먼저 적고 중간 결과를 출력하거나 로그로 남겨 추측을 줄인다. 그다음 값 하나, 호출 순서 하나, 경계 조건 하나만 바꿔 CHAPTER 07 · boundary translation은 내부 exception vocabulary를 public domain vocabulary로 바꾼다의 규칙이 실제 상태에 어떤 변화를 만드는지 확인한다. PART 74 CHAPTER 7에서는 결과를 맞히는 것보다 예상과 실제의 차이를 설명하는 과정이 중요하다. 설명이 맞더라도 반대 조건을 넣어 경계가 어디인지 다시 확인한다. P74-C07의 통과 기준은 결과를 외우는 것이 아니다. P74-C07의 동작을 타입, 정체성, 수명, 오류 경계로 나눠 설명하고 비슷한 코드에서도 동일한 규칙을 찾아낼 수 있어야 한다.
