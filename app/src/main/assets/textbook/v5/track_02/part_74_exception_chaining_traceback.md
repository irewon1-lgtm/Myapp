...
**검증 시나리오 P74-C1 — CHAPTER 01 · exception context는 처리 중 새 exception이 발생했음을 연결한다**
`CHAPTER 01 · exception context는 처리 중 새 exception이 발생했음을 연결한다` 검증은 정상 경로를 먼저 재현하는 데서 시작한다. P74-C1에서는 `CHAPTER 01 · exception context는 처리 중 새 exception이 발생했음을 연결한다` 실행 직전 상태를 먼저 적고 실행 뒤 값과 비교해 실제 규칙을 확인한다. 두 번째 단계에서는 `CHAPTER 01 · exception context는 처리 중 새 exception이 발생했음을 연결한다`에 대해 오류 경로 하나를 의도적으로 만든다고 다른 코드는 그대로 두어 원인 후보를 하나로 제한한다. 이때 `CHAPTER 01 · exception context는 처리 중 새 exception이 발생했음을 연결한다`의 예외 종류와 직전 상태를 함께 남긴다하여 우연한 통과를 배제한다. 예상과 다르면 `CHAPTER 01 · exception context는 처리 중 새 exception이 발생했음을 연결한다`의 타입, 값, 호출 순서, 예외 경계 가운데 최초로 달라진 항목부터 확인하고 최소 수정 뒤 다시 실행한다. P74-C1의 마무리는 복구 후 같은 오류가 다시 재현되지 않는지 검사하는 것이다. 통과 기준은 `CHAPTER 01 · exception context는 처리 중 새 exception이 발생했음을 연결한다`의 정상 사례와 변형 사례가 모두 설명 가능한 결과를 내고 같은 절차를 반복해도 동일한 상태 계약을 유지하는 것이다.
...
**검증 시나리오 P74-C2 — CHAPTER 02 · `raise ... from ...`은 명시적 cause를 만들어 번역 관계를 표현한다**
`CHAPTER 02 · `raise ... from ...`은 명시적 cause를 만들어 번역 관계를 표현한다` 검증은 호출 순서를 단순화하는 데서 시작한다. P74-C2에서는 `CHAPTER 02 · `raise ... from ...`은 명시적 cause를 만들어 번역 관계를 표현한다` 실행 직전 상태를 먼저 적고 실행 뒤 값과 비교해 실제 규칙을 확인한다. 두 번째 단계에서는 `CHAPTER 02 · `raise ... from ...`은 명시적 cause를 만들어 번역 관계를 표현한다`에 대해 순서 하나만 뒤집어 차이를 본다고 다른 코드는 그대로 두어 원인 후보를 하나로 제한한다. 이때 `CHAPTER 02 · `raise ... from ...`은 명시적 cause를 만들어 번역 관계를 표현한다`의 호출 전후의 상태 전이를 번호로 남긴다하여 우연한 통과를 배제한다. 예상과 다르면 `CHAPTER 02 · `raise ... from ...`은 명시적 cause를 만들어 번역 관계를 표현한다`의 타입, 값, 호출 순서, 예외 경계 가운데 최초로 달라진 항목부터 확인하고 최소 수정 뒤 다시 실행한다. P74-C2의 마무리는 다른 순서에서도 계약이 유지되는지 확인하는 것이다. 통과 기준은 `CHAPTER 02 · `raise ... from ...`은 명시적 cause를 만들어 번역 관계를 표현한다`의 정상 사례와 변형 사례가 모두 설명 가능한 결과를 내고 같은 절차를 반복해도 동일한 상태 계약을 유지하는 것이다.
...
**검증 시나리오 P74-C3 — CHAPTER 03 · `from None`은 context 표시를 억제하지만 원인 자체를 없애는 설계 도구는 아니다**
`CHAPTER 03 · `from None`은 context 표시를 억제하지만 원인 자체를 없애는 설계 도구는 아니다` 검증은 입력 크기를 고정하는 데서 시작한다. P74-C3에서는 `CHAPTER 03 · `from None`은 context 표시를 억제하지만 원인 자체를 없애는 설계 도구는 아니다` 실행 직전 상태를 먼저 적고 실행 뒤 값과 비교해 실제 규칙을 확인한다. 두 번째 단계에서는 `CHAPTER 03 · `from None`은 context 표시를 억제하지만 원인 자체를 없애는 설계 도구는 아니다`에 대해 변형은 한 요소만 허용고 다른 코드는 그대로 두어 원인 후보를 하나로 제한한다. 이때 `CHAPTER 03 · `from None`은 context 표시를 억제하지만 원인 자체를 없애는 설계 도구는 아니다`의 측정값을 여러 번 모아 분포를 비교하여 우연한 통과를 배제한다. 예상과 다르면 `CHAPTER 03 · `from None`은 context 표시를 억제하지만 원인 자체를 없애는 설계 도구는 아니다`의 타입, 값, 호출 순서, 예외 경계 가운데 최초로 달라진 항목부터 확인하고 최소 수정 뒤 다시 실행한다. P74-C3의 마무리는 워밍업과 측정 자체의 비용을 분리하는 것이다. 통과 기준은 `CHAPTER 03 · `from None`은 context 표시를 억제하지만 원인 자체를 없애는 설계 도구는 아니다`의 정상 사례와 변형 사례가 모두 설명 가능한 결과를 내고 같은 절차를 반복해도 동일한 상태 계약을 유지하는 것이다.
...
**검증 시나리오 P74-C5 — CHAPTER 05 · bare `raise`는 현재 exception의 traceback을 보존하는 재발생 경로다**
`CHAPTER 05 · bare `raise`는 현재 exception의 traceback을 보존하는 재발생 경로다` 검증은 반례부터 하나 만든다하는 데서 시작한다. P74-C5에서는 `CHAPTER 05 · bare `raise`는 현재 exception의 traceback을 보존하는 재발생 경로다` 실행 직전 상태를 먼저 적고 실행 뒤 값과 비교해 실제 규칙을 확인한다. 두 번째 단계에서는 `CHAPTER 05 · bare `raise`는 현재 exception의 traceback을 보존하는 재발생 경로다`에 대해 반례와 정상례의 차이만 좁힌다고 다른 코드는 그대로 두어 원인 후보를 하나로 제한한다. 이때 `CHAPTER 05 · bare `raise`는 현재 exception의 traceback을 보존하는 재발생 경로다`의 중간 값을 직접 출력해 추측을 줄인다하여 우연한 통과를 배제한다. 예상과 다르면 `CHAPTER 05 · bare `raise`는 현재 exception의 traceback을 보존하는 재발생 경로다`의 타입, 값, 호출 순서, 예외 경계 가운데 최초로 달라진 항목부터 확인하고 최소 수정 뒤 다시 실행한다. P74-C5의 마무리는 수정 후 두 사례를 같은 순서로 재검증하는 것이다. 통과 기준은 `CHAPTER 05 · bare `raise`는 현재 exception의 traceback을 보존하는 재발생 경로다`의 정상 사례와 변형 사례가 모두 설명 가능한 결과를 내고 같은 절차를 반복해도 동일한 상태 계약을 유지하는 것이다.
...
**검증 시나리오 P74-C7 — CHAPTER 07 · boundary translation은 내부 exception vocabulary를 public domain vocabulary로 바꾼다**
`CHAPTER 07 · boundary translation은 내부 exception vocabulary를 public domain vocabulary로 바꾼다` 검증은 가장 작은 객체 상태로 시작하는 데서 시작한다. P74-C7에서는 `CHAPTER 07 · boundary translation은 내부 exception vocabulary를 public domain vocabulary로 바꾼다` 실행 직전 상태를 먼저 적고 실행 뒤 값과 비교해 실제 규칙을 확인한다. 두 번째 단계에서는 `CHAPTER 07 · boundary translation은 내부 exception vocabulary를 public domain vocabulary로 바꾼다`에 대해 속성 하나만 바꿔 재실행고 다른 코드는 그대로 두어 원인 후보를 하나로 제한한다. 이때 `CHAPTER 07 · boundary translation은 내부 exception vocabulary를 public domain vocabulary로 바꾼다`의 반환값과 부수효과를 따로 기록하여 우연한 통과를 배제한다. 예상과 다르면 `CHAPTER 07 · boundary translation은 내부 exception vocabulary를 public domain vocabulary로 바꾼다`의 타입, 값, 호출 순서, 예외 경계 가운데 최초로 달라진 항목부터 확인하고 최소 수정 뒤 다시 실행한다. P74-C7의 마무리는 수정 뒤 원래 조건을 다시 회귀 확인하는 것이다. 통과 기준은 `CHAPTER 07 · boundary translation은 내부 exception vocabulary를 public domain vocabulary로 바꾼다`의 정상 사례와 변형 사례가 모두 설명 가능한 결과를 내고 같은 절차를 반복해도 동일한 상태 계약을 유지하는 것이다.
