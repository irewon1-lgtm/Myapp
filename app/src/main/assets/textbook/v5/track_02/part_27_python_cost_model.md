...
**검증 시나리오 P27-C10 — CHAPTER 10 · 최적화 우선순위는 `알고리즘 → I/O → 표현 → 미세 비용` 순으로 좁힌다**
`CHAPTER 10 · 최적화 우선순위는 `알고리즘 → I/O → 표현 → 미세 비용` 순으로 좁힌다` 검증은 입력 크기를 고정하는 데서 시작한다. P27-C10에서는 `CHAPTER 10 · 최적화 우선순위는 `알고리즘 → I/O → 표현 → 미세 비용` 순으로 좁힌다` 실행 직전 상태를 먼저 적고, 실행 뒤 얻은 값과 비교해 어떤 규칙이 실제로 적용됐는지 확인한다. 두 번째 단계에서는 `CHAPTER 10 · 최적화 우선순위는 `알고리즘 → I/O → 표현 → 미세 비용` 순으로 좁힌다`에 대해 변형은 한 요소만 허용고, 다른 코드는 그대로 두어 원인 후보를 하나로 제한한다. 이때 `CHAPTER 10 · 최적화 우선순위는 `알고리즘 → I/O → 표현 → 미세 비용` 순으로 좁힌다`의 측정값을 여러 번 모아 분포를 비교하여 결과만 맞는 우연한 통과를 배제한다. 예상이 빗나가면 `CHAPTER 10 · 최적화 우선순위는 `알고리즘 → I/O → 표현 → 미세 비용` 순으로 좁힌다`의 타입, 값, 호출 순서, 예외 경계 가운데 최초로 달라진 항목부터 확인하고 최소 수정 뒤 다시 실행한다. P27-C10의 마무리는 워밍업과 측정 자체의 비용을 분리하는 것이다. 통과 기준은 `CHAPTER 10 · 최적화 우선순위는 `알고리즘 → I/O → 표현 → 미세 비용` 순으로 좁힌다`의 정상 사례와 변형 사례가 모두 설명 가능한 결과를 내고, 같은 절차를 반복했을 때 동일한 상태 계약을 유지하는 것이다.
