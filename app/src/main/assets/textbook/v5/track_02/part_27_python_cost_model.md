...
**측정 점검 27-10 — CHAPTER 10 · 최적화 우선순위는 `알고리즘 → I/O → 표현 → 미세 비용` 순으로 좁힌다**
CHAPTER 10 · 최적화 우선순위는 `알고리즘 → I/O → 표현 → 미세 비용` 순으로 좁힌다을 성능 관점에서 확인할 때는 관찰할 변수를 두세 개로 줄이고 준비 단계와 측정 구간을 분리한다. 실행 전 예상값과 실행 후 실제값을 나란히 기록한다. PART 27 CHAPTER 10에서는 입력 크기와 반복 횟수를 고정한 뒤 여러 번 측정해 분포를 보고, 대안 코드와 비교할 때는 한 요소만 바꾼다. 코드 모양보다 값의 변화와 객체의 수명을 중심으로 원인을 좁힌다. 속도 차이를 자료구조·할당·호출 횟수·워밍업 가운데 어느 요인으로 설명할 수 있는지까지 적어야 측정 자체의 오류를 피할 수 있다.
