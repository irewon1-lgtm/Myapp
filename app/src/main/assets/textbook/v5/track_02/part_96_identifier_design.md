...
CHAPTER 02 · random identifier는 중앙 coordination 없이 새 identity를 만들기 쉽다을 검증할 때는 제한시간과 재시도 횟수를 명시하고 연속 실패 뒤 정상 요청을 보낸다. PART 96 CHAPTER 2의 핵심 관찰값으로 중복 반영과 누락을 별도 지표로 센다. 실패를 주입한 뒤 정상 경로로 돌아왔을 때 같은 입력을 다시 처리해 멱등성, 복구 가능성, 상태 일관성을 확인한다. 수정 전후를 비교할 때는 성공 여부만 보지 말고 재시도 횟수, 중복 처리, 누락, 남은 작업을 함께 본다. 통과 기준은 반복 실행에서도 결과가 안정적이고, 실패가 생겨도 손실이나 무한 반복 없이 추적 가능한 상태로 종료되는 것이다.
