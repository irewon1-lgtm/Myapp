...
**운영 검증 100-2 — CHAPTER 02 · signal handler는 복잡한 cleanup을 직접 수행하기보다 종료 state를 시작한다**
CHAPTER 02 · signal handler는 복잡한 cleanup을 직접 수행하기보다 종료 state를 시작한다을 검증할 때는 성공 기준을 수치나 상태로 정하고 처리 중간에 프로세스를 중단한다. PART 100 CHAPTER 2의 핵심 관찰값으로 대기열 길이와 거부된 요청 수를 같이 본다. P100-C02의 실패 주입 결과는 다음 정상 실행과 연결해서 본다. 같은 입력을 다시 처리할 때 P100-C02의 상태 머신이 이전 오류를 끌고 가지 않고 정해진 완료 상태로 돌아오는지 확인한다. P100-C02: 변경 효과는 성공 플래그보다 중복 반영 수와 미처리 항목 수로 판단한다. 재실행 뒤 누락이 줄고 이미 완료된 항목이 다시 처리되지 않으며 종료 상태가 명확해야 한다.
...
**운영 검증 100-5 — CHAPTER 05 · resource close order는 dependency graph의 역순으로 정한다**
CHAPTER 05 · resource close order는 dependency graph의 역순으로 정한다을 검증할 때는 부분 성공을 구분할 상태 값을 정하고 중복 요청을 한 번 넣는다. PART 100 CHAPTER 5의 핵심 관찰값으로 재시도 로그와 실제 반영 건수를 비교한다. P100-C05에서는 장애를 제거한 뒤 동일 입력으로 기준 결과를 다시 만든다. P100-C05 재실행에서 완료 항목이 중복되지 않고 실패 항목만 정책대로 이어지며 최종 상태가 정상 실행과 같아야 한다. P100-C05: 회귀 확인에서는 정상 실행과 장애 실행의 상태 전이 수를 각각 기록한다. 재시도 상한 안에서 종료되고 남은 작업이 추적 가능하며 데이터 개수가 기준 실행과 같아야 한다.
...
**운영 검증 100-6 — CHAPTER 06 · queue는 drain·requeue·discard 중 어떤 정책을 쓸지 명시한다**
CHAPTER 06 · queue는 drain·requeue·discard 중 어떤 정책을 쓸지 명시한다을 검증할 때는 제한시간과 재시도 횟수를 명시하고 연속 실패 뒤 정상 요청을 보낸다. PART 100 CHAPTER 6의 핵심 관찰값으로 중복 반영과 누락을 별도 지표로 센다. P100-C06 복구 단계에서는 동일한 식별자로 요청을 재전송한다. P100-C06의 이미 완료된 효과는 유지되고 미완료 부분만 이어지며, 재실행 결과가 최초 정상 경로와 같은 최종 상태로 수렴해야 한다. P100-C06: 수정 전후 비교표에는 성공 건수, 실패 건수, 재시도 수, 최종 대기열 길이를 넣는다. 반복 실행에서 이 값들이 수렴하고 손실이나 무한 재시도가 나타나지 않아야 한다.
