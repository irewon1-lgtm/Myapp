...
Rate limit은 “초당 몇 번” 숫자 하나로 끝나지 않는다. 초당 요청 수가 낮아도 각 요청이 오래 걸리면 동시 실행 수가 폭증할 수 있고, 평균 rate가 같아도 짧은 burst가 downstream을 무너뜨릴 수 있다. 또한 limit 초과 요청을 queue에 쌓을지 즉시 거절할지에 따라 latency와 memory가 달라진다. 이 절에서는 **rate·concurrency·burst·queue를 별도 자원으로 분리**한다.
...
**현장 디버깅 점검 — CHAPTER 01 · rate와 concurrency는 서로 다른 부하 차원이다**
CHAPTER 01 · rate와 concurrency는 서로 다른 부하 차원이다을 운영 환경에서 다룰 때는 정상 경로뿐 아니라 중단·재시도·부분 성공을 함께 검증해야 한다. 먼저 동일한 입력과 초기 상태로 재현 절차를 고정하고 기대 상태를 기록한다. 그다음 지연, 중복 요청, 잘못된 식별자, 만료된 제한시간, 부분 실패 중 한 조건만 주입한다. 로그에는 요청 식별자와 상태 전이, 재시도 횟수, 최종 반환값을 남겨 원인을 추적할 수 있게 한다. 수정 후에는 정상 입력과 실패 입력을 같은 순서로 다시 실행한다. 통과 기준은 결과가 한 번 맞는 것이 아니라 손실·중복·무한 재시도 없이 종료되고, 다시 실행해도 같은 계약을 지키는 것이다.
...
**현장 디버깅 점검 — CHAPTER 02 · token bucket은 평균 rate와 제한된 burst를 함께 표현한다**
CHAPTER 02 · token bucket은 평균 rate와 제한된 burst를 함께 표현한다을 운영 환경에서 다룰 때는 정상 경로뿐 아니라 중단·재시도·부분 성공을 함께 검증해야 한다. 먼저 동일한 입력과 초기 상태로 재현 절차를 고정하고 기대 상태를 기록한다. 그다음 지연, 중복 요청, 잘못된 식별자, 만료된 제한시간, 부분 실패 중 한 조건만 주입한다. 로그에는 요청 식별자와 상태 전이, 재시도 횟수, 최종 반환값을 남겨 원인을 추적할 수 있게 한다. 수정 후에는 정상 입력과 실패 입력을 같은 순서로 다시 실행한다. 통과 기준은 결과가 한 번 맞는 것이 아니라 손실·중복·무한 재시도 없이 종료되고, 다시 실행해도 같은 계약을 지키는 것이다.
...
**현장 디버깅 점검 — CHAPTER 05 · client-side limiter는 downstream을 보호하면서 자기 queue도 bounded하게 해야 한다**
CHAPTER 05 · client-side limiter는 downstream을 보호하면서 자기 queue도 bounded하게 해야 한다을 운영 환경에서 다룰 때는 정상 경로뿐 아니라 중단·재시도·부분 성공을 함께 검증해야 한다. 먼저 동일한 입력과 초기 상태로 재현 절차를 고정하고 기대 상태를 기록한다. 그다음 지연, 중복 요청, 잘못된 식별자, 만료된 제한시간, 부분 실패 중 한 조건만 주입한다. 로그에는 요청 식별자와 상태 전이, 재시도 횟수, 최종 반환값을 남겨 원인을 추적할 수 있게 한다. 수정 후에는 정상 입력과 실패 입력을 같은 순서로 다시 실행한다. 통과 기준은 결과가 한 번 맞는 것이 아니라 손실·중복·무한 재시도 없이 종료되고, 다시 실행해도 같은 계약을 지키는 것이다.
...
**현장 디버깅 점검 — CHAPTER 07 · overload에서 queue와 reject 중 무엇을 선택할지 latency budget으로 판단한다**
CHAPTER 07 · overload에서 queue와 reject 중 무엇을 선택할지 latency budget으로 판단한다을 운영 환경에서 다룰 때는 정상 경로뿐 아니라 중단·재시도·부분 성공을 함께 검증해야 한다. 먼저 동일한 입력과 초기 상태로 재현 절차를 고정하고 기대 상태를 기록한다. 그다음 지연, 중복 요청, 잘못된 식별자, 만료된 제한시간, 부분 실패 중 한 조건만 주입한다. 로그에는 요청 식별자와 상태 전이, 재시도 횟수, 최종 반환값을 남겨 원인을 추적할 수 있게 한다. 수정 후에는 정상 입력과 실패 입력을 같은 순서로 다시 실행한다. 통과 기준은 결과가 한 번 맞는 것이 아니라 손실·중복·무한 재시도 없이 종료되고, 다시 실행해도 같은 계약을 지키는 것이다.
