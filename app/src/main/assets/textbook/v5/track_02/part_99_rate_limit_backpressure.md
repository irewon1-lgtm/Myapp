...
CHAPTER 01 · rate와 concurrency는 서로 다른 부하 차원이다을 검증할 때는 재시작 전후를 비교할 값을 고르고 부분 배치를 실패시킨다. PART 99 CHAPTER 1의 핵심 관찰값으로 실패 원인과 복구 결과가 같은 식별자로 이어지는지 확인한다. 실패를 주입한 뒤 정상 경로로 돌아왔을 때 같은 입력을 다시 처리해 멱등성, 복구 가능성, 상태 일관성을 확인한다. 수정 전후를 비교할 때는 성공 여부만 보지 말고 재시도 횟수, 중복 처리, 누락, 남은 작업을 함께 본다. 통과 기준은 반복 실행에서도 결과가 안정적이고, 실패가 생겨도 손실이나 무한 반복 없이 추적 가능한 상태로 종료되는 것이다.
...
CHAPTER 02 · token bucket은 평균 rate와 제한된 burst를 함께 표현한다을 검증할 때는 요청과 처리 상태를 연결할 식별자를 정하고 같은 식별자로 다시 전달한다. PART 99 CHAPTER 2의 핵심 관찰값으로 종료 전 남은 작업과 종료 후 상태를 비교한다. 실패를 주입한 뒤 정상 경로로 돌아왔을 때 같은 입력을 다시 처리해 멱등성, 복구 가능성, 상태 일관성을 확인한다. 수정 전후를 비교할 때는 성공 여부만 보지 말고 재시도 횟수, 중복 처리, 누락, 남은 작업을 함께 본다. 통과 기준은 반복 실행에서도 결과가 안정적이고, 실패가 생겨도 손실이나 무한 반복 없이 추적 가능한 상태로 종료되는 것이다.
...
CHAPTER 05 · client-side limiter는 downstream을 보호하면서 자기 queue도 bounded하게 해야 한다을 검증할 때는 기대 상태를 먼저 적고 응답 지연을 일부러 만든다. PART 99 CHAPTER 5의 핵심 관찰값으로 체크포인트와 저장된 데이터의 위치를 맞춘다. 실패를 주입한 뒤 정상 경로로 돌아왔을 때 같은 입력을 다시 처리해 멱등성, 복구 가능성, 상태 일관성을 확인한다. 수정 전후를 비교할 때는 성공 여부만 보지 말고 재시도 횟수, 중복 처리, 누락, 남은 작업을 함께 본다. 통과 기준은 반복 실행에서도 결과가 안정적이고, 실패가 생겨도 손실이나 무한 반복 없이 추적 가능한 상태로 종료되는 것이다.
...
CHAPTER 07 · overload에서 queue와 reject 중 무엇을 선택할지 latency budget으로 판단한다을 검증할 때는 제한시간과 재시도 횟수를 명시하고 연속 실패 뒤 정상 요청을 보낸다. PART 99 CHAPTER 7의 핵심 관찰값으로 중복 반영과 누락을 별도 지표로 센다. 실패를 주입한 뒤 정상 경로로 돌아왔을 때 같은 입력을 다시 처리해 멱등성, 복구 가능성, 상태 일관성을 확인한다. 수정 전후를 비교할 때는 성공 여부만 보지 말고 재시도 횟수, 중복 처리, 누락, 남은 작업을 함께 본다. 통과 기준은 반복 실행에서도 결과가 안정적이고, 실패가 생겨도 손실이나 무한 반복 없이 추적 가능한 상태로 종료되는 것이다.
