...
**운영 검증 99-1 — CHAPTER 01 · rate와 concurrency는 서로 다른 부하 차원이다**
CHAPTER 01 · rate와 concurrency는 서로 다른 부하 차원이다을 검증할 때는 재시작 전후를 비교할 값을 고르고 부분 배치를 실패시킨다. PART 99 CHAPTER 1의 핵심 관찰값으로 실패 원인과 복구 결과가 같은 식별자로 이어지는지 확인한다. P99-C01에서는 장애를 제거한 뒤 동일 입력으로 기준 결과를 다시 만든다. P99-C01 재실행에서 완료 항목이 중복되지 않고 실패 항목만 정책대로 이어지며 최종 상태가 정상 실행과 같아야 한다. P99-C01: 변경 후에는 중복·누락·재시도·미완료 작업을 각각 별도 지표로 센다. 여러 번 실행해도 각 지표가 예상 범위에 머물고 종료 조건이 항상 동일하면 통과다.
...
**운영 검증 99-2 — CHAPTER 02 · token bucket은 평균 rate와 제한된 burst를 함께 표현한다**
CHAPTER 02 · token bucket은 평균 rate와 제한된 burst를 함께 표현한다을 검증할 때는 요청과 처리 상태를 연결할 식별자를 정하고 같은 식별자로 다시 전달한다. PART 99 CHAPTER 2의 핵심 관찰값으로 종료 전 남은 작업과 종료 후 상태를 비교한다. P99-C02 복구 단계에서는 동일한 식별자로 요청을 재전송한다. P99-C02의 이미 완료된 효과는 유지되고 미완료 부분만 이어지며, 재실행 결과가 최초 정상 경로와 같은 최종 상태로 수렴해야 한다. P99-C02: 수정 효과를 검증할 때는 처리 완료 목록과 남은 목록을 직접 대조한다. 재시도 이후 완료 목록이 불필요하게 늘지 않고 실패 목록만 줄어드는 방향으로 움직여야 한다.
...
**운영 검증 99-5 — CHAPTER 05 · client-side limiter는 downstream을 보호하면서 자기 queue도 bounded하게 해야 한다**
CHAPTER 05 · client-side limiter는 downstream을 보호하면서 자기 queue도 bounded하게 해야 한다을 검증할 때는 기대 상태를 먼저 적고 응답 지연을 일부러 만든다. PART 99 CHAPTER 5의 핵심 관찰값으로 체크포인트와 저장된 데이터의 위치를 맞춘다. P99-C05 장애 주입 뒤에는 같은 입력을 다시 넣어 P99-C05의 중복 처리 여부와 복구 시작점을 함께 비교한다. 재실행이 이전 성공분을 다시 반영하지 않고 남은 작업만 이어지면 상태 일관성이 유지된 것이다. P99-C05: 회귀 테스트는 성공률 하나로 끝내지 않는다. 동일 입력의 재실행 결과, 재시도 횟수, 중복 반영 여부, 종료 상태를 함께 확인해 모든 값이 계약 범위 안에 있어야 한다.
...
**운영 검증 99-7 — CHAPTER 07 · overload에서 queue와 reject 중 무엇을 선택할지 latency budget으로 판단한다**
CHAPTER 07 · overload에서 queue와 reject 중 무엇을 선택할지 latency budget으로 판단한다을 검증할 때는 제한시간과 재시도 횟수를 명시하고 연속 실패 뒤 정상 요청을 보낸다. PART 99 CHAPTER 7의 핵심 관찰값으로 중복 반영과 누락을 별도 지표로 센다. P99-C07 실패 후 재처리는 멱등성 확인 단계다. P99-C07의 처리 횟수·최종 상태·재시도 로그를 함께 비교해 같은 입력이 여러 번 와도 데이터 의미가 변하지 않는지 검증한다. P99-C07: 수정 전후는 처리 건수와 재시도 횟수를 분리해 비교한다. 같은 입력을 세 번 반복했을 때 최종 반영 수가 변하지 않고 실패 이유가 동일한 식별자로 남으면 검증을 통과한다.
