...
**검증 시나리오 P66-C04 — CHAPTER 04 · slicing은 하나의 index가 아니라 범위 표현 객체를 전달한다**
`CHAPTER 04 · slicing은 하나의 index가 아니라 범위 표현 객체를 전달한다` 검증은 성공 사례를 기준선으로 저장하는 데서 시작한다. P66-C4에서는 `CHAPTER 04 · slicing은 하나의 index가 아니라 범위 표현 객체를 전달한다` 실행 직전 상태를 먼저 적고, 실행 뒤 얻은 값과 비교해 어떤 규칙이 실제로 적용됐는지 확인한다. 두 번째 단계에서는 `CHAPTER 04 · slicing은 하나의 index가 아니라 범위 표현 객체를 전달한다`에 대해 실패 조건은 하나만 주입고, 다른 코드는 그대로 두어 원인 후보를 하나로 제한한다. 이때 `CHAPTER 04 · slicing은 하나의 index가 아니라 범위 표현 객체를 전달한다`의 로그 시각과 상태 식별자를 맞춰 본다하여 결과만 맞는 우연한 통과를 배제한다. 예상이 빗나가면 `CHAPTER 04 · slicing은 하나의 index가 아니라 범위 표현 객체를 전달한다`의 타입, 값, 호출 순서, 예외 경계 가운데 최초로 달라진 항목부터 확인하고 최소 수정 뒤 다시 실행한다. P66-C4의 마무리는 성공·실패 모두 결정적으로 끝나는지 확인하는 것이다. 통과 기준은 `CHAPTER 04 · slicing은 하나의 index가 아니라 범위 표현 객체를 전달한다`의 정상 사례와 변형 사례가 모두 설명 가능한 결과를 내고, 같은 절차를 반복했을 때 동일한 상태 계약을 유지하는 것이다.
