...
**실행 점검 66-4 — CHAPTER 04 · slicing은 하나의 index가 아니라 범위 표현 객체를 전달한다**
CHAPTER 04 · slicing은 하나의 index가 아니라 범위 표현 객체를 전달한다을 이해할 때는 경계값 한 개를 골라 성공 경로와 오류 경로의 차이를 줄 단위로 추적한다. 이어서 값 하나나 호출 순서 하나만 바꿔 결과 차이를 확인하면 CHAPTER 04 · slicing은 하나의 index가 아니라 범위 표현 객체를 전달한다이 실제 프로그램 상태에 어떤 제약을 주는지 보인다. PART 66 CHAPTER 4의 한 줄 해석은 “규칙을 값과 상태 변화로 확인한다”이다. 같은 증상이 다시 나오도록 재현 절차를 남긴 뒤 원인을 수정한다. 마지막에는 타입·정체성·수명·예외 경계 중 실제 원인이 어디였는지 자기 문장으로 설명해, 단순한 문법 복사가 아니라 동작 원리까지 이해했는지 확인한다.
