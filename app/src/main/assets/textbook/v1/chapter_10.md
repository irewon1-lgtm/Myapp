...
성능 작업은 숫자가 조금 좋아졌다고 끝나지 않는다. 목표 응답시간이나 frame budget처럼 **사용자가 체감하는 완료 기준**을 먼저 정하고, 대표 데이터 규모와 실제 기기 조건에서 반복 측정해야 한다. 개선 후 메모리 사용량이나 배터리 소모, 데이터 최신성이 악화됐다면 단순한 속도 향상을 성공으로 승인하지 않는다. 측정 로그와 회귀시험을 남겨 다음 버전에서도 같은 기준으로 다시 확인할 수 있어야 한다.
