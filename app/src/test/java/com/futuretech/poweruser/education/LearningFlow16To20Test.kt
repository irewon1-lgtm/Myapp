...
class LearningFlow16To20Test {
    private fun note(type: String, count: Int): PersonalErrorNoteEntity = PersonalErrorNoteEntity(
        id = count.toLong(), errorType = type, codeSnippet = "sample", errorMessage = type,
        correctionGuide = "fix $type", occurrenceCount = count,
    ...
    fun frequentRealErrorGetsMoreChallengeSlots() {
        ...
        val weighted = ErrorDrivenPracticeEngine.challengeFlow(listOf(note("FOCUSED_DEBUG", 8), note("FOCUSED_FILL", 1)))
    ...
    fun reviewVariantChangesInputInsteadOfRepeatingSameCard() {
        val item = SpacedRepetitionItemEntity(
            conceptId = "x", conceptTitle = "변수", category = "BEGINNER",
            definition = "값을 이름으로 저장한다.", analogy = "상자",
            example = "score = 10\nprint(score)", comparison = "상수와 구분",
            reviewIntervalDays = 3, nextReviewTimestamp = 0L, reviewCount = 2, isMastered = false
        )
