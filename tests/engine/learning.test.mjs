...
test('새 기본 작업은 한국어 충실 기준본과 선택적 초보자 설명 프로파일',async()=>{const {job}=await setup();assert.equal(job.engineVersion,'1.1.0');assert.equal(job.config.profile,E.PROFILE);assert.equal(job.config.externalCallsEnabled,false);assert.equal(job.config.maxCostUsd,0);assert.equal(job.learningPolicy.minPages,null);});
...
test('통과 결과 승인·내보내기와 정책 연결',async()=>{const{job,ch}=await readyDraft();await E.submit(job,ch,'review',review(job,ch),{actor:'reviewer'});await E.approve(job,'test-owner');const b=await E.exportBook(job);assert.equal(b._engine.version,'1.1.0');assert.equal(b.chapters.length,1);});
