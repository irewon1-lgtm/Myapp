(()=>{'use strict';const C=window.CB;
 ...
 C.offlineAll=async()=>{if(window.CHATBOOK_PORTABLE||location.protocol==='file:'){C.toast('이 실행파일에 모든 기본 도서와 표지가 이미 포함돼 있어요.');return;}if(!('caches'in window))throw Error('이 브라우저는 오프라인 보관을 지원하지 않아요. 서재 백업을 이용해 주세요.');C.toast('책과 앱 화면을 이 기기에 저장하고 있어요.',6000);const reg=await Promise.race([navigator.serviceWorker?.ready,new Promise((_,reject)=>setTimeout(()=>reject(Error('오프라인 보관을 시작하지 못했어요. 새로고침 후 다시 시도해 주세요.')),12000))]);const cache=await caches.open('chatbook-offline-v1');await cache.addAll(window.CHATBOOK_CACHE_URLS||['./','./index.html','./style.css','./library.js','./core.js','./views.js','./reader.js','./exports.js','./app.js','./vendor/pptxgen.bundle.js',...new Set(C.catalog.books.map(b=>'./art/'+b.cover+'.webp'))]);await C.filePut('catalog',C.catalog);try{await navigator.storage?.persist?.();}catch{}C.set('offline:saved',Date.now());C.toast('오프라인 서재를 저장했어요. 연결이 끊겨도 다시 읽을 수 있어요.',5000);};
 ...
 document.addEventListener('visibilitychange',()=>{if(document.hidden){C.Reader.persist();return;}if(C.pref('autoBooks',true))C.updateBooks();if(C.pref('sync',false)&&C.secret())C.sync().catch(()=>{});});
