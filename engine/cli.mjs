...
try{
 if(action==='prepare'){
   ...
   const job=await newJob(source);
   const policy=JSON.parse(fs.readFileSync(new URL('../public/engine/learning-policy.json',import.meta.url),'utf8'));
   pinAuthoring(job,{version:VERSION,profile:PROFILE,policy:POLICY},policy);
   await store.save(job);console.log(JSON.stringify({jobId:job.id,chunks:source.chunks,warnings:source.warnings},null,2));
 }else{
   const job=await store.load();
   if(action==='spec')console.log(JSON.stringify(readPinnedAuthoring(job),null,2));
   else if(action==='status')console.log(JSON.stringify({status:job.status,gate:publicationGate(job),completed:Object.keys(job.checkpoints)},null,2));
   else if(action==='task')console.log(JSON.stringify({...task(job,args[0],args[1]),authoringDigest:job.authoringSpec?.contentDigest||null,authoringInstruction:'Read the saved rules once with: node engine/cli.mjs spec JOB_DIR. Do not silently replace them with a newer website spec.'},null,2));
