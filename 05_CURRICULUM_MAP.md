...
2. 실행 전 결과 예상
3. 코드/구조 읽기 문제
4. 한 부분 직접 수정하고 실제 실행
5. 핵심 개념 빈칸 문제
6. 보지 않고 짧은 코드 다시 작성
7. 일부러 고장난 예제 디버깅
8. AI의 그럴듯한 오답 판별
9. 원본을 바꿔 새 상황에 적용하는 작은 응용 미션
10. 본인 말로 설명 + 핵심개념 포함률 기반 숙련 확인

힌트는 방향 → 문제 위치 → 거의 정답 순으로 단계적으로 제공한다. 정답을 처음부터 표시하지 않는다. 실패한 빈칸/직접작성/실행/디버깅/AI 판별/응용 미션은 오류노트에 누적하고, 완료한 개념은 1·3·7·14일 복습 대상으로 등록한다.
